...
class SubprocVecEnv(ShareVecEnv):
    ...
    def reset(self, fix_z=None):
        if fix_z is None:
            ...
            for (remote, z) in zip(self.remotes, fix_z):
                remote.send(('reset', z))
    ...
    def seed(self, seed_num):
        for remote, s in zip(self.remotes, seed_num):
            remote.send(('seed', s))
...
class DummyVecEnv(ShareVecEnv):
    ...
    def reset(self, fix_z=None):
        obs = [env.reset(fix_z) for env in self.envs]
        return np.array(obs)
...
class ShareDummyVecEnv(ShareVecEnv):
    ...
    def reset(self, set_z=None):
        results = [env.reset(set_z) for env in self.envs]
